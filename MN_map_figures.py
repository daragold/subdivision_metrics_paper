...
mn_plan_merge = mn_plan_merge.merge(subdiv_splits_mn_plans.rename(columns = {plan:'SPLITS_'+plan for plan in [col for col in subdiv_splits_mn_plans.columns if col!=geoid and 'COURT_ORDERED' not in col]}), how = 'left', on = geoid)
...
for plan in ['COURT_ORDERED']+[col for col in mn_plan_merge.columns if 'SPLITS_' in col.upper()]:
